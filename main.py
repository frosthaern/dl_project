...
def strong_train_tf():
    # RandomErasing moved before Normalize to keep erasing in image space
    return T.Compose([
...
# --------------------------
# Mixup/CutMix helper (with guards)
# --------------------------
...
def apply_mixup_cutmix(x, y, prob=0.0, mixup_alpha=0.8, cutmix_alpha=1.0, cutmix_prob=0.5):
    ...
    If no mix applied -> lam=1.0 and (y,y,1.0)
    """
    ...
    if use_cut:
        ...
        # if degenerate box (no area) fallback to mixup:
        if x2 <= x1 or y2 <= y1:
...
# --------------------------
# Training loop (single function) - returns train_loss, train_unmixed_acc, mixed_fraction
# --------------------------
...
def train_epoch(model, loader, optimizer, criterion, scaler, mix_cfg):
    ...
    for xb, yb in tqdm(loader, leave=False):
        ...
        xb_aug, (ya, yb2, lam) = apply_mixup_cutmix(
            xb, yb,
        ...
        # Only count unmixed examples for training accuracy (to avoid misleading numbers)
        if lam == 1.0:
            correct_unmixed += preds.eq(yb).sum().item()
            unmixed_total += yb.size(0)

    # final train metrics
    avg_loss = running_loss / total_examples
    unmixed_acc = (correct_unmixed / unmixed_total) if unmixed_total > 0 else (correct_unmixed / total_examples)
    mixed_fraction = 1.0 - (unmixed_total / total_examples)
    return avg_loss, unmixed_acc, mixed_fraction
...
def run_experiment(name, builder, train_tf, test_tf, epochs, batch_size, lr, wd, mix_cfg, use_ema=False, ema_decay=0.999):
    ...
    history = {"train_loss": [], "train_acc": [], "train_mixed_frac": [], "val_loss": [], "val_acc": []}
    start_epoch = 0
    ...
    for epoch in range(start_epoch + 1, epochs + 1):
        ep_start = time.time()
        tr_loss, tr_unmixed_acc, mixed_frac = train_epoch(model, train_loader, optimizer, criterion, scaler, mix_cfg)
        val_loss, val_acc, _, _ = eval_model(model, test_loader, criterion, collect_preds=False)

        history["train_loss"].append(tr_loss); history["train_acc"].append(tr_unmixed_acc)
        history["train_mixed_frac"].append(mixed_frac)
        ...
        # optionally save EMA shadow to checkpoint (as CPU tensors) for resume reproducibility
        if ema is not None:
        ...
        logger.info(f"Epoch {epoch}: tr_loss={tr_loss:.4f}, tr_unmixed_acc={tr_unmixed_acc*100:.2f}%, val_loss={val_loss:.4f}, val_acc={val_acc*100:.2f}%, mixed_frac={mixed_frac:.2f}")
        print(f"[{name}] Epoch {epoch}/{epochs} done — val_acc={val_acc*100:.2f}%, unmixed_train_acc={tr_unmixed_acc*100:.2f}%, mixed_frac={mixed_frac:.2f}, epoch_time={epoch_time:.1f}s, ETA={eta/60:.1f}min")
